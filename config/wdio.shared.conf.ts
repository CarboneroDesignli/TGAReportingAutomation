...
export const config: WebdriverIO.Config = {
    ...
    before: async function () {
        await driver.startRecordingScreen({ timeLimit: 300 });
    },
    after: async function () {
        try {
            const recording = await driver.stopRecordingScreen();
            if (recording) {
                allure.addAttachment('Screen Recording', Buffer.from(recording, 'base64'), 'video/mp4');
            }
        } catch (err) {
            console.warn('Failed to attach screen recording:', err);
        }
    },
